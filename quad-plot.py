"""Plot solutions for Stokes flow at quadrature points."""
...
ddmg = np.clip(interpolate(ddmg, SQ).vector().array(), 0, np.inf)

# pore pressure
pHD = pHD.vector().array()
pHD /= 1e3  # convert to kPa

# effective pressure
prs = interpolate(p, SQ).vector().array()
prs /= 1e3  # convert to kPa
...
def save_plot(name, data, filename):
    ...
    try:
        ...
        if len(data) == len(xQ1):
            plot = plt.tricontourf(xQ1, xQ2, data, 360, cmap=rainbow, vmin=f0,
                                   vmax=f1)
        else:
            plot = plt.tricontourf(x1, x2, tri, data, 360, cmap=rainbow,
                                   vmin=f0, vmax=f1)
